#' Generating Multivariate Sieves with Sine Polynomials
#' @description The \code{sine.gen} command is used to generate multivariate sieves with sine polynomials. It considers the interactions among sine polynomial bases of different variables in a dataset.
#' In particular, if a user defines a test dataset which he hopes to use for his own model evaluation, the \code{sine.gen} command also generates the corresponding sieves of
...
#' @param n.basis a non-negative integer that specifies the number of sine polynomial basis for each variable. If \code{NULL}, it will be calculated according to the formulas established in this \href{https://github.com/ccfang2/Masters_Thesis}{thesis}.
...
#'
#' @return \code{sine.gen} generates multivariate sieves with sine polynomials, considering the interactions between basis of different variables. If \code{test.data} is supplied,
...
#' for the evaluation of model estimated with sieves of main dataset. The data frame in the output of \code{sine.gen} could be directly used as an input of the argument \code{x.sieve} in command \code{\link{dimada}}.
...
#' # sieves with sine polynomials, considering no interactions of these 3 variables
#' sieve1 <- sine.gen(data=df, test.data=NULL, n.basis=10, max.interaction=1)
#'
...
#' sieve2 <- sine.gen(data=df, test.data=df.test, n.basis=10, max.interaction=1)
sine.gen <- function(data,
                     test.data=NULL,
                     n.basis=NULL,
                     max.interaction=1)
  ...
  index.table <- index.table[-1, ,drop=FALSE] # there is no intercept in sine polynomials
  colnames(index.table) <- var.names

  # --------------------------------
  # generate names for sine polynomial terms
  # --------------------------------
  ...
  terms.names.gen <- function(index.row) {
    ...
    for (i in 1:length(index.row)) {
      if (index.row[i] != 0){
        name.temp <- base::paste(var.names[i],index.row[i], sep=".sin")
  ...
  # --------------------------------
  # compute values for sine polynomial terms
  # --------------------------------

  if (!is.null(test.data)) {
    sines <- plyr::llply(1:ncol(data),function(x) cbind(1,t(plyr::ldply(1:n.basis, function(y) sin(y*pi*data[,x])))))
    terms.values.int <- base::apply(index.table,1, function(x) plyr::llply(1:ncol(index.table), function(y) sines[[y]][,(x[y]+1)]))
    terms.values <- as.data.frame(t(plyr::ldply(terms.values.int, function(x) Reduce("*", x))))

    sines.test <- plyr::llply(1:ncol(test.data),function(x) cbind(1,t(plyr::ldply(1:n.basis, function(y) sin(y*pi*test.data[,x])))))
    terms.values.int.test <- base::apply(index.table,1, function(x) plyr::llply(1:ncol(index.table), function(y) sines.test[[y]][,(x[y]+1)]))
