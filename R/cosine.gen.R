#' Generating Multivariate Sieves with Cosine Polynomials
#' @description The \code{cosine.gen} command is used to generate multivariate sieves with cosine polynomials. It considers the interactions among cosine polynomial bases of different variables in a dataset.
#' In particular, if a user defines a test dataset which he hopes to use for his own model evaluation, the \code{cosine.gen} command also generates the corresponding sieves of
...
#' @param n.basis a non-negative integer that specifies the number of cosine polynomial bases for each variable. If \code{NULL}, it will be calculated according to the formulas established in this \href{https://github.com/ccfang2/Masters_Thesis}{thesis}.
...
#'
#' @return \code{cosine.gen} generates multivariate sieves with cosine polynomials, considering the interactions between basis of different variables. If \code{test.data} is supplied,
...
#' for the evaluation of model estimated with sieves of main dataset. The data frame in the output of \code{cosine.gen} could be directly used as an input of the argument \code{x.sieve} in command \code{\link{dimada}}.
#' @seealso \link{poly.gen}; \link{bspline.gen}; \link{sine.gen}; \link{trig.gen}; \link{haar.gen}; \link{daubechies.gen}; \link{dimada}.
#' @export
#'
#' @note An internal function \code{create_index_matrix} in package \pkg{Sieve} is applied to help with the generation of multivariate sieves in function \code{cosine.gen}.
#'
...
#' # sieves with cosine polynomials, considering full interactions of these 3 variables
#' sieve1 <- cosine.gen(data=df, test.data=NULL, n.basis=10, max.interaction=3)
#'
...
#' sieve2 <- cosine.gen(data=df, test.data=df.test, n.basis=10, max.interaction=3)
cosine.gen <- function(data,
                       test.data=NULL,
                       n.basis=NULL,
                       max.interaction=1)
  {
  ...
  # --------------------------------
  # generate names for cosine polynomial terms
  # --------------------------------
  ...
  terms.names.gen <- function(index.row) {
    if (sum(index.row)==0) {
      ...
      for (i in 1:length(index.row)) {
        if (index.row[i] != 0){
          name.temp <- base::paste(var.names[i],index.row[i], sep=".cos")
  ...
  # --------------------------------
  # compute values for cosine polynomial terms
  # --------------------------------

  if (!is.null(test.data)) {
    cosines <- plyr::llply(1:ncol(data),function(x) cbind(1,t(plyr::ldply(1:n.basis, function(y) cos(y*pi*data[,x])))))
    terms.values.int <- base::apply(index.table,1, function(x) plyr::llply(1:ncol(index.table), function(y) cosines[[y]][,(x[y]+1)]))
    terms.values <- as.data.frame(t(plyr::ldply(terms.values.int, function(x) Reduce("*", x))))

    cosines.test <- plyr::llply(1:ncol(test.data),function(x) cbind(1,t(plyr::ldply(1:n.basis, function(y) cos(y*pi*test.data[,x])))))
    terms.values.int.test <- base::apply(index.table,1, function(x) plyr::llply(1:ncol(index.table), function(y) cosines.test[[y]][,(x[y]+1)]))
